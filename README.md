...
| keycloak | 9095 | OAuth2/OIDC сервер, realm: `bank-realm`. Доступен из браузера (LoadBalancer) |
...
Keycloak доступен из браузера на `http://localhost:9095` (порт проброшен наружу через LoadBalancer). Пользователи могут обращаться только к данным своего аккаунта: все backend-сервисы проверяют `preferred_username` в JWT-токене.
...
| Трейсинг | Zipkin, Micrometer Tracing (Brave) |
| Мониторинг | Prometheus, Grafana |
| Логирование | ELK (Elasticsearch + Logstash + Kibana), Logback + LogstashEncoder |
...
Ожидаемое состояние: 13 подов в `Running` (включая `bank-app-kafka-0` StatefulSet) + 1 `Completed` (keycloak-init Job).
...
Тесты проверяют доступность actuator/health каждого сервиса, соединение с PostgreSQL/Kafka и работоспособность Zipkin/Prometheus/Grafana. Все 13 тестов должны пройти со статусом `Succeeded`.

Приложение: **http://localhost:8084**

### 5. Запустить ELK-стек (логирование)

ELK развёрнут отдельно от Kubernetes, через Docker Compose:
...
После запуска Kibana автоматически создаёт data view `bank-logs-*`. Логи сервисов станут доступны в Kibana через ~30 секунд.

| Инструмент | URL | Описание |
|---|---|---|
| Zipkin | http://localhost:9411 | Распределённые трассировки |
| Prometheus | http://localhost:9090 | Метрики сервисов |
| Grafana | http://localhost:3000 | Дашборды и алерты (admin/admin) |
| Kibana | http://localhost:5601 | Логи сервисов (data view: `bank-logs-*`) |

### Наблюдаемость

#### Трейсинг (Zipkin)

Все 7 сервисов и фронт автоматически поставляют трассировки в Zipkin (http://localhost:9411) с помощью Micrometer Tracing + Brave:
- Трассируются все входящие и исходящие HTTP-запросы (B3-заголовки)
- Трассируются запросы к PostgreSQL (через R2DBC Proxy — accounts, auth)
- Трассируются сообщения в Apache Kafka (producer и consumer)
- Sampling: 100% (`management.tracing.sampling.probability=1.0`)

#### Мониторинг (Prometheus + Grafana)

Prometheus (http://localhost:9090) скрейпит метрики всех 7 сервисов каждые 15 секунд (`/actuator/prometheus`).

Grafana (http://localhost:3000, admin/admin) содержит 3 преднастроенных дашборда:
- **Bank HTTP Metrics** — RPS, 4xx/5xx ошибки, перцентили задержек (p50/p95/p99)
- **Bank JVM Metrics** — Heap-память, потоки, CPU, GC
- **Bank Business Metrics** — Неуспешные снятия средств, переводы, отправки уведомлений

Настроены алерты (Prometheus + Grafana) по превышению пороговых значений:
- High HTTP 5xx Error Rate
- High Request Latency (p95 > 2s)
- Cash Withdrawal Failures (> 5 в 5 минут)
- Transfer Failures (> 5 в 5 минут)
- Notification Send Failures (> 3 в 5 минут)
- JVM Heap Memory High (> 85%)
- Service Down

Кастомные метрики сервисов:
- `cash_withdrawal_failures_total{login}` — неуспешные снятия наличных
- `transfer_failures_total{from_login, to_login}` — неуспешные переводы
- `notification_send_failures_total{login}` — ошибки обработки уведомлений

#### Логирование (ELK)

Все сервисы пишут логи в JSON-формате через `LogstashTcpSocketAppender` в Logstash (порт 5000). Logstash маскирует пароли и 20-значные номера счетов, затем отправляет в Elasticsearch (индексы `bank-logs-YYYY.MM.dd`).

В Kibana (http://localhost:5601) доступен data view `bank-logs-*`. Каждый лог содержит поля `traceId` и `spanId` для связи логов с трассировками в Zipkin.

Управление ELK:

```bash
make elk-up    # запустить ELK
make elk-down  # остановить ELK
make elk-logs  # просмотр логов ELK-компонентов
```
...
│           ├── postgres/
│           ├── kafka/
│           ├── zipkin/           # Zipkin (трейсинг, inside K8s)
│           ├── prometheus/       # Prometheus (мониторинг, inside K8s)
│           ├── grafana/          # Grafana (дашборды и алерты, inside K8s)
│           ├── elk/              # ExternalName services для ELK (outside K8s)
│           └── common/           # library chart — shared deployment template
├── elk/                    # ELK-стек (outside K8s): docker-compose + logstash pipeline
