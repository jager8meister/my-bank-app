...
class CashServiceTest {
    ...
    void shouldProcessPutOperationSuccessfully() {
        ...
        when(responseSpec.bodyToMono(Integer.class)).thenReturn(Mono.just(5500));
        ...
        StepVerifier.create(result)
                .assertNext(response -> {
                    assertThat(response.newBalance()).isEqualTo(5500);
    ...
    void shouldProcessGetOperationSuccessfully() {
        ...
        when(requestBodyUriSpec.uri(any(java.util.function.Function.class))).thenReturn(requestBodySpec);
        when(requestBodySpec.retrieve()).thenReturn(responseSpec);
        when(responseSpec.bodyToMono(Integer.class)).thenReturn(Mono.just(4500));
        when(notificationClient.sendCashOperationNotification(anyString(), anyString(), anyString()))
                .thenReturn(Mono.empty());
        Mono<CashResponse> result = cashService.processCashOperation("ivanov", request);
        StepVerifier.create(result)
                .assertNext(response -> {
                    assertThat(response.newBalance()).isEqualTo(4500);
    ...
    void shouldUseCorrectEndpointForPutOperation() {
        ...
        when(requestBodyUriSpec.uri(any(java.util.function.Function.class))).thenReturn(requestBodySpec);
        when(requestBodySpec.retrieve()).thenReturn(responseSpec);
        when(responseSpec.bodyToMono(Integer.class)).thenReturn(Mono.just(6000));
        when(notificationClient.sendCashOperationNotification(anyString(), anyString(), anyString()))
                .thenReturn(Mono.empty());
    ...
    void shouldUseCorrectEndpointForGetOperation() {
        CashOperationRequest request = new CashOperationRequest(1000, CashAction.GET);
        when(webClient.post()).thenReturn(requestBodyUriSpec);
        when(requestBodyUriSpec.uri(any(java.util.function.Function.class))).thenReturn(requestBodySpec);
        when(requestBodySpec.retrieve()).thenReturn(responseSpec);
        when(responseSpec.bodyToMono(Integer.class)).thenReturn(Mono.just(2000));
        when(notificationClient.sendCashOperationNotification(anyString(), anyString(), anyString()))
                .thenReturn(Mono.empty());
    ...
    void shouldHandleInsufficientFundsInErrorMessage() {
        CashOperationRequest request = new CashOperationRequest(5000, CashAction.GET);
        when(webClient.post()).thenReturn(requestBodyUriSpec);
        when(requestBodyUriSpec.uri(any(java.util.function.Function.class))).thenReturn(requestBodySpec);
        when(requestBodySpec.retrieve()).thenReturn(responseSpec);
        when(responseSpec.bodyToMono(Integer.class))
    ...
    void shouldProcessLargeDepositAmount() {
        CashOperationRequest request = new CashOperationRequest(1000000, CashAction.PUT);
        when(webClient.post()).thenReturn(requestBodyUriSpec);
        when(requestBodyUriSpec.uri(any(java.util.function.Function.class))).thenReturn(requestBodySpec);
        when(requestBodySpec.retrieve()).thenReturn(responseSpec);
        when(responseSpec.bodyToMono(Integer.class)).thenReturn(Mono.just(1005000));
        when(notificationClient.sendCashOperationNotification(anyString(), anyString(), anyString()))
                .thenReturn(Mono.empty());
        Mono<CashResponse> result = cashService.processCashOperation("ivanov", request);
        StepVerifier.create(result)
                .assertNext(response -> {
                    assertThat(response.newBalance()).isEqualTo(1005000);
    ...
    void shouldProcessMinimumValidAmount() {
        CashOperationRequest request = new CashOperationRequest(1, CashAction.PUT);
        when(webClient.post()).thenReturn(requestBodyUriSpec);
        when(requestBodyUriSpec.uri(any(java.util.function.Function.class))).thenReturn(requestBodySpec);
        when(requestBodySpec.retrieve()).thenReturn(responseSpec);
        when(responseSpec.bodyToMono(Integer.class)).thenReturn(Mono.just(5001));
        when(notificationClient.sendCashOperationNotification(anyString(), anyString(), anyString()))
                .thenReturn(Mono.empty());
        Mono<CashResponse> result = cashService.processCashOperation("ivanov", request);
        StepVerifier.create(result)
                .assertNext(response -> {
                    assertThat(response.newBalance()).isEqualTo(5001);
                })
                .verifyComplete();
    }
}
